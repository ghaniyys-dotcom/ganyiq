...
# ── FASE 11: Module-level resources ──
_F11_VALIDATOR = None

def _get_f11_validator():
    """Lazy-initialize FASE 11 face mesh validator (module-level singleton)."""
    global _F11_VALIDATOR
    if _F11_VALIDATOR is None:
        try:
            import mediapipe as mp_v
            _F11_VALIDATOR = mp_v.solutions.face_mesh.FaceMesh(
                static_image_mode=True, max_num_faces=1, min_detection_confidence=0.3
            )
        except Exception:
            _F11_VALIDATOR = False
    return _F11_VALIDATOR if _F11_VALIDATOR is not False else None
...
def process_video(
    ...
    while frame_idx < end_frame:
        ...
        if (frame_idx - start_frame) % frame_interval == 0:
            ...
            # ── FASE 11: Landmark geometry validation ──
            validated_yolo = []
            if enable_mediapipe:
                try:
                    _f11_v = _get_f11_validator()
                    if _f11_v is None:
                        ...
                        for face in yolo_faces:
                            ...
                            results = _f11_v.process(rgb_crop)
                            if results and results.multi_face_landmarks:
                                face_lms = results.multi_face_landmarks[0].landmark
                                if len(face_lms) >= 14:
                                    frame_h_c, frame_w_c = face_crop.shape[:2]
                                    p_left_eye = (int(face_lms[33].x * frame_w_c), int(face_lms[33].y * frame_h_c)) if 33 < len(face_lms) else None
                                    p_right_eye = (int(face_lms[263].x * frame_w_c), int(face_lms[263].y * frame_h_c)) if 263 < len(face_lms) else None
                                    p_nose = (int(face_lms[1].x * frame_w_c), int(face_lms[1].y * frame_h_c)) if 1 < len(face_lms) else None
                                    p_mouth = (int(face_lms[13].x * frame_w_c), int(face_lms[13].y * frame_h_c)) if 13 < len(face_lms) else None
                                    if all([p_left_eye, p_right_eye, p_nose, p_mouth]):
                                        eye_dist = np.linalg.norm(np.array(p_left_eye) - np.array(p_right_eye))
                                        nose_mouth_dist = np.linalg.norm(np.array(p_nose) - np.array(p_mouth))
                                        if eye_dist >= 5 and nose_mouth_dist >= 3:
                                            ratio = eye_dist / max(nose_mouth_dist, 1)
                                            if 0.6 < ratio < 4.0:
                                                validated_yolo.append(face)
                                                continue
                                validated_yolo.append(face)
                            else:
                                pass
